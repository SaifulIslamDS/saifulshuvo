...
`ADMIN_EMAILS` is the preferred server-only comma-separated allow-list; `ADMIN_EMAIL` remains backward compatible. The callback rejects and signs out any Google account whose normalized email does not match.
...
- RLS separately verifies `public.is_portfolio_admin()`.
- Public users can select published projects only.
- Permanent deletion requires `publication_status = 'archived'` in both application logic and RLS.
- Project audit events are generated by a database trigger rather than trusted client input.
